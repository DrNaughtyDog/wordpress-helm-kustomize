...
Configure the needed values in values-trimmed.yaml
e.g.
ingress.hostname for access or
image.registry and image.repository if you use a dedicated registry or
ingressClassName to define the IngressController to use

```yaml
helm upgrade -i wordpress-helm -f values-trimmed.yaml .
```
